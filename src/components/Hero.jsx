    ...
    <section className="relative min-h-screen flex flex-col justify-center px-6 md:px-12 lg:px-20 pt-24 pb-16 max-w-7xl mx-auto">
      ...
      >
        ...
        <motion.h1
          ...
          className="font-serif text-5xl md:text-7xl lg:text-8xl text-cream mb-4 leading-none tracking-tight"
        >
          Esraa Elnagdi
        </motion.h1>
        ...
        <motion.h2
          variants={itemVariants}
          className="font-serif text-3xl md:text-5xl lg:text-6xl text-cream/90 mb-3 leading-tight"
        >
        ...
        <motion.p
          variants={itemVariants}
          className="font-serif italic text-2xl md:text-3xl text-amber-lab mb-8 leading-tight"
        >
        ...
        <motion.p
          variants={itemVariants}
          className="font-sans text-base md:text-lg text-cream-muted max-w-2xl leading-relaxed mb-10"
        >
        ...
        >
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-0 md:divide-x md:divide-cream/10">
            {metrics.map((m, i) => (
              <div key={i} className="md:px-8 first:pl-0 last:pr-0">
                <div className="font-serif text-4xl md:text-5xl text-amber-lab leading-none mb-1">
                  {m.value}
                  {m.unit && (
                    <span className="text-2xl md:text-3xl text-amber-lab/70">{m.unit}</span>
                  )}
                </div>
                <div className="font-mono text-[10px] tracking-[0.12em] uppercase text-cream-dark leading-tight mt-1">
                  {m.label}
                </div>
              </div>
            ))}
      ...
      >
        <span className="font-mono text-[9px] tracking-[0.25em] uppercase text-cream-dark rotate-90 origin-center translate-x-4">
          scroll
        </span>
        <div className="w-px h-12 bg-gradient-to-b from-cream-dark/40 to-transparent" />
      </motion.div>
    </section>
  );
}
